...
class AssignmentCNNModel(tf.keras.Model):
    ...
    def call(self, inputs):
        # Inputs: (product_input, matrix_input, product_mask)
        if len(inputs) == 3:
        ...
        # Apply mask: set scores of fake products to large negative value
        mask_expanded = tf.expand_dims(product_mask, axis=-1)  # (batch, num_products, 1)
        scores = scores * mask_expanded + (1 - mask_expanded) * (-1e9)

        assignment_probs = tf.nn.softmax(scores, axis=-1)

        # Optional: build preliminary grid for visualization/debug
